...
class Consumer:
    ...
    async def update_task_status(self, task_id: str, status: str, message: str = None):
        ...
        try:
            # 更新Redis中的任务状态
            self.redis.hset(f"task_info:{task_id}", "status", status)
            if message:
                self.redis.hset(f"task_info:{task_id}", "message", message)
    ...
    async def download_video(self, task_id: str, url: str) -> str:
        ...
        if not video_path:
            error_msg = f"【Consumer】- 视频下载失败: task_id={task_id}, url={url}"
            logger.error(error_msg)
            raise Exception(error_msg)
        download_time = round(time.time() - download_start, 3)
        logger.info(f"【Consumer】- 视频下载成功: {video_path}, 耗时={download_time}秒")
        return video_path

    async def generate_video_tags(self, task_id: str, video_path: str) -> dict:
        """生成视频标签"""
        logger.info(f"【Consumer】- 解析视频标签开始: task_id={task_id}")
        ...
        try:
            ...
            error_msg = (
                f"【Consumer】- 生成视频标签失败: task_id={task_id}, error={str(e)}"
            )
            logger.error(error_msg)
            raise Exception(error_msg)
        vision_time = round(time.time() - vision_start, 3)
        logger.info(
            f"【Consumer】- 获取视频标签成功: task_id={task_id}, 耗时={vision_time}秒"
        )

        return json.loads(vision_response.strip())

    async def sync_tags_to_es(self, task_id: str, material_id: str, tags: dict) -> None:
        """同步标签到ES服务"""
        sync_start = time.time()
        logger.info(f"【Consumer】- 开始同步标签到ES: task_id={task_id}")
        try:
            # 验证tags数据格式
            if not isinstance(tags, dict):
                error_msg = (
                    f"【Consumer】- 标签数据格式错误，期望dict类型: task_id={task_id}"
                )
                logger.error(error_msg)
                raise ValueError(error_msg)

            if not tags:
                error_msg = f"【Consumer】- 标签数据为空: task_id={task_id}"
                logger.error(error_msg)
                raise ValueError(error_msg)

            es_api_url = os.getenv("ES_API_URL")
            if not es_api_url:
                error_msg = f"【Consumer】- ES_API_URL环境变量未配置，请检查.env文件: task_id={task_id}"
                logger.error(error_msg)
                raise Exception(error_msg)

            # 构造请求数据
            material_ids = json.loads(material_id) if material_id else []
            ...
            logger.info(
                f"【Consumer】- 同步标签到ES成功: task_id={task_id}, 耗时={sync_time}秒"
            )

        except requests.exceptions.Timeout:
            error_msg = f"【Consumer】- ES入库服务请求超时: task_id={task_id}"
            logger.error(error_msg)
            raise Exception(error_msg)
        except requests.exceptions.RequestException as e:
            error_msg = (
                f"【Consumer】- ES入库服务请求异常: task_id={task_id}, error={str(e)}"
            )
            logger.error(error_msg)
            raise Exception(error_msg)
        except ValueError as e:
            error_msg = f"【Consumer】- ES入库服务响应解析失败: task_id={task_id}, error={str(e)}"
            logger.error(error_msg)
            raise Exception(error_msg)
    ...
    async def update_mysql_tags(self, task_id: str, tags: dict) -> None:
        ...
        if task:
            try:
                ...
                logger.info(
                    f"【Consumer】- DB更新标签成功: task_id={task_id}, 耗时={update_time}秒"
                )
            except Exception as e:
                error_msg = f"【Consumer】- 更新MySQL标签失败: task_id={task_id}, error={str(e)}"
                logger.error(error_msg)
                raise Exception(error_msg)
    ...
    async def process_task(self, task_id: str):
        ...
        try:
            ...
            while retry_count < 3:
                try:
                    task_info = self.redis.hgetall(f"task_info:{task_id}")
                    break
                except Exception as e:
                    retry_count += 1
                    if retry_count == 3:
                        raise
                    logger.warning(
                        f"【Consumer】- 获取任务信息重试 {retry_count}/3: {str(e)}"
                    )
                    await asyncio.sleep(1)

            if not task_info:
                logger.error(f"【Consumer】- 任务 {task_id} 不存在")
                return

            # 更新任务状态为处理中
            await self.update_task_status(task_id, "processing")
            logger.info(
                f"【Consumer】- 开始处理任务: {task_id}, 视频URL: {task_info.get('url')}"
            )
            ...
            try:
                ...
                logger.info(
                    f"【Consumer】- 任务处理完成: task_id={task_id}, 总耗时={total_time}秒"
                )

                # 删除Redis中的任务信息
                self.redis.delete(f"task_info:{task_id}")

            except Exception as e:
                logger.error(f"【Consumer】- 处理任务 {task_id} 失败: {str(e)}")
                ...
                if retry_count >= self.max_retries:
                    ...
                    logger.error(
                        f"【Consumer】- 任务 {task_id} 达到最大重试次数({self.max_retries})，移入失败队列"
                    )
                else:
                    # 重新加入队列
                    self.redis.lpush("task_queue", task_id)
                    logger.warning(
                        f"【Consumer】- 任务 {task_id} 重试次数: {retry_count}/{self.max_retries}"
                    )
                raise

        except Exception as e:
            logger.error(f"【Consumer】- 处理任务 {task_id} 时发生错误: {str(e)}")
            await self.update_task_status(task_id, "failed", str(e))
            ...
            if video_path and os.path.exists(video_path):
                try:
                    ...
                    logger.info(f"【Consumer】- 清理临时文件成功: {video_path}")
                except Exception as e:
    ...
    async def run(self):
        """启动消费者服务"""
        logger.info("【Consumer】- 启动视频标签处理消费者服务")
        while True:
            try:
                # 获取任务
                ...
                if not await self.acquire_lock(task_id):
                    logger.warning(f"【Consumer】- 任务 {task_id} 正在被其他进程处理")
                    continue
                ...
                logger.error(f"【Consumer】- 消费者服务发生错误: {str(e)}")
                await asyncio.sleep(1)
    ...
    async def main(cls):
        ...
        try:
            ...
            logger.error(f"【Consumer】- 启动消费者失败: {str(e)}")
            raise
        finally:
            # 关闭数据库连接
            if db:
